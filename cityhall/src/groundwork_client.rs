//! HTTP-backed implementation of `plan::GroundworkLookup`.
//!
//! Reads the Groundwork REST API. The base URL comes from the `GROUNDWORK_URL`
//! env var (default `http://localhost:3000`).

use crate::plan::{DeployableSummary, GroundworkLookup};
...
impl GroundworkLookup for HttpGroundworkClient {
    async fn get_deployable(&self, id: &str) -> anyhow::Result<Option<DeployableSummary>> {
        ...
        // Pull dependencies via /dependency/graph getByDeployableId.
        let dep_query =
        ...
        // For each service, find the deployable that exposes it (if any).
